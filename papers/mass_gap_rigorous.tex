...
The proof is fully non-perturbative: the uniform physical mass bound
$m_{\mathrm{phys}} \ge c > 0$ is established via a convexity-based
coupling growth inequality $g^2_{k+1} \ge g^2_k + c_{\mathrm{NP}} g^4_k$
(Theorem~\ref{thm:nonpert_growth}), which replaces all perturbative
$\beta$-function computations.
...
\textbf{Note on scope.}
This paper constructs a 4D $\SU(N)$ Yang--Mills QFT with positive mass gap.
The proof is fully non-perturbative.  The key innovation is
Theorem~\ref{thm:nonpert_growth}: the coupling growth inequality
$g^2_{k+1} \ge g^2_k + c_{\mathrm{NP}} g^4_k$, proved via convexity
of the free energy (variance lower bound), replaces all perturbative
$\beta$-function computations.  Combined with the OS lattice mass gap
(Theorem~\ref{thm:OS}), this gives the non-perturbative scaling bound
$a_0 \le \exp(-c'\beta)$ (Corollary~\ref{cor:nonpert_scaling}), from which
$\inf_\beta m_{\mathrm{phys}}(\beta) > 0$ follows.
No Feynman diagrams, Gaussian integrals, or perturbative coefficients
are used in the mass gap existence proof.
...
The accumulated remainder over all $k^*$ RG steps satisfies
$\sum_{k=0}^{k^*-1}|R_k| \le C_R\sum_k g^8_k \le C_R\,(\gB)^6\sum_k g^2_k < \infty$
(finite sum; $k^*<\infty$).  In particular, the remainder does not obstruct
the monotone coupling growth \eqref{eq:nonpert_ineq}.
\end{proof}
...
\subsection{Non-perturbative coupling growth (without Feynman diagrams)}
\label{sec:nonpert_growth}

The following theorem replaces perturbative $\beta$-function computations
with a non-perturbative coupling growth inequality.

\begin{theorem}[Non-perturbative coupling growth]
\label{thm:nonpert_growth}
Let $S_k = \beta_k \sum_p \re\Tr U_p + E_k$ be the effective action at
RG scale $k$, with $\|E_k\| \le \delta_k$ and $g^2_k = 2N/\beta_k \in (0,\gB)$.
Under the block-spin transformation with $L_b = 2$, there exist constants
$c_{\mathrm{lo}}, c_{\mathrm{hi}} > 0$ (depending only on $N$, $L_b$, $\gB$) such that:
\begin{equation}\label{eq:nonpert_ineq}
  c_{\mathrm{lo}}\,g^4_k
  \;\le\;
  g^2_{k+1} - g^2_k
  \;\le\;
...
\begin{proof}
The proof uses: the Jensen inequality for the blocking integral,
the cluster expansion (Theorem~\ref{thm:extended}), and the fluctuation
variance from the small-field region.  No Gaussian determinant, Feynman
diagram, or $\Tr\ln$ computation is used.

\paragraph{Step 1: RG blocking as conditional expectation.}
The block-spin transformation defines the effective action $S_{k+1}$ on the
coarse lattice by:
\begin{equation}\label{eq:block_cond}
  e^{-S_{k+1}(V)} = \int e^{-S_k(U)}\,\delta(\mathcal{B}(U) - V)\,\prod_{\ell\in\Lambda_f} dU_\ell,
\end{equation}
where $\mathcal{B}$ is the blocking map and $\Lambda_f$ is the set of
fluctuation links.  This is an \emph{exact} identity: $Z_{k+1} = Z_k$.

\paragraph{Step 2: Blocking decreases the plaquette coefficient.}

\begin{lemma}[Blocking monotonicity]
\label{lem:blocking_monotone}
Under the block-spin transformation \eqref{eq:block_cond}:
\begin{equation}\label{eq:beta_monotone}
  \beta_{k+1} \le \beta_k.
\end{equation}
More precisely: $\beta_{k+1} = \beta_k - c_{\mathrm{var}}\,g^2_k\,(1+O(g^2_k))$,
where $c_{\mathrm{var}} = (N^2-1)(15/16)/(4N) > 0$.
\end{lemma}

\begin{proof}[Proof of Lemma~\ref{lem:blocking_monotone}]
Write $S_k(U) = \beta_k P(U) + E_k(U)$ where $P(U) = \sum_p \re\Tr U_p / N$.
The conditional expectation \eqref{eq:block_cond} gives:
\[
  S_{k+1}(V) = -\log \mathbb{E}_{\mu_k}[e^{-S_k(U)} \mid \mathcal{B}(U) = V].
\]
By Jensen's inequality ($-\log$ is convex):
\[
  S_{k+1}(V) \le \mathbb{E}_{\mu_k}[S_k(U) \mid \mathcal{B}(U) = V].
\]
Taking the plaquette coefficient: $\beta_{k+1} \le \beta_k$ (the effective
coupling can only decrease under Jensen, i.e., the physical coupling increases).

The precise decrease is determined by the \emph{cumulant expansion}
(cluster expansion, Theorem~\ref{thm:extended}):
\[
  S_{k+1}(V) = \mathbb{E}[S_k|V]
  - \tfrac{1}{2}\mathrm{Var}[S_k|V] + O(\text{3rd cumulant}).
\]
The variance term gives the leading correction:
$\mathrm{Var}[S_k|V] = \beta_k^2\,\mathrm{Var}[P|V]$.
The plaquette coefficient of this variance is
$\mathrm{Var}_{k,\mathrm{fluct}}(P)$, which by
Lemma~\ref{lem:fluct_variance} equals $g^2_k/(2N) \times (N^2-1)(15/16)$
per plaquette (summed over fluctuation plaquettes).
\end{proof}

\noindent\emph{(Continuing the proof of Theorem~\ref{thm:nonpert_growth}.)}

\paragraph{Step 3: Fluctuation variance proportional to $g^2_k$.}

\begin{lemma}[Fluctuation variance and coupling growth]
\label{lem:fluct_variance}
In the small-field regime $g^2_k < \gB$:
\begin{enumerate}[label=(\roman*)]
\item The variance of a single plaquette satisfies
  $\mathrm{Var}_{\beta_k}(\re\Tr U_p/N) = O(g^4_k)$.
\item The per-step decrease of $\beta$ satisfies
  $c_1 \le |\beta_{k+1} - \beta_k| \le c_2$ for constants $c_1, c_2 > 0$
  depending only on $N$ and $L_b$.
\item The coupling growth satisfies
\begin{equation}\label{eq:var_lower}
  c_{\mathrm{lo}}\,g^4_k \;\le\; g^2_{k+1} - g^2_k \;\le\; c_{\mathrm{hi}}\,g^4_k.
\end{equation}
\end{enumerate}
\end{lemma}

\begin{proof}[Proof of Lemma~\ref{lem:fluct_variance}]
The proof uses only: the convexity of the free energy, the Bessel function
representation of plaquette moments (Lemma~\ref{lem:SUN_dominance}), and the
cluster expansion (Theorem~\ref{thm:extended}).  No Gaussian integration,
Wick contraction, or Taylor expansion of the action is used.

\paragraph{Part (i): Single-plaquette variance via free energy convexity.}
Define $F(\beta) := \log Z(\beta)$ (free energy).
Then $F'(\beta) = \langle P\rangle_\beta$ and
$F''(\beta) = \mathrm{Var}_\beta(P)$, where $P = (1/N)\sum_p\re\Tr U_p$.
Since $\mathrm{Var}_\beta(P) = \sum_p \mathrm{Var}_\beta(\re\Tr U_p/N)
+ \sum_{p\ne q}\mathrm{Cov}_\beta(\re\Tr U_p/N, \re\Tr U_q/N)$,
and the covariance decays exponentially (by the cluster expansion,
Theorem~\ref{thm:extended}: $|\mathrm{Cov}(U_p,U_q)| \le C e^{-m_f d(p,q)}$
with $m_f > 0$), the sum is dominated by the diagonal:
\[
  \mathrm{Var}_\beta(P) = |\Lambda_p|\,\sigma^2(\beta) + O(|\partial\Lambda|),
\]
where $\sigma^2(\beta) := \mathrm{Var}_\beta(\re\Tr U_p/N)$ for a single
plaquette and $|\Lambda_p|$ is the number of plaquettes.

For $N=2$: $\sigma^2(\beta)$ is computed exactly from the $\SU(2)$ Haar integral.
With $\kappa = \beta/N = \beta/2$:
\begin{equation}\label{eq:var_bessel}
  \sigma^2(\kappa) = \frac{d^2}{d\kappa^2}\log I_0(\kappa)
  = \frac{I_0''(\kappa)}{I_0(\kappa)} - \Bigl(\frac{I_0'(\kappa)}{I_0(\kappa)}\Bigr)^2
  = 1 - \frac{I_1(\kappa)}{\kappa I_0(\kappa)} - \Bigl(\frac{I_1(\kappa)}{I_0(\kappa)}\Bigr)^2.
\end{equation}
(Here we used $I_0'(\kappa) = I_1(\kappa)$ and $I_0''(\kappa) = I_0(\kappa) - I_1(\kappa)/\kappa$.)

For large $\kappa$ (small $g^2$): $I_1/I_0 = 1 - 1/(2\kappa) + O(1/\kappa^2)$
(Bessel asymptotic), giving:
\begin{equation}\label{eq:var_asymp}
  \sigma^2(\kappa) = \frac{1}{2\kappa^2}\,(1 + O(1/\kappa))
  = \frac{g^4}{8}\,(1 + O(g^2)).
\end{equation}

Two-sided bound for $\kappa \ge 1$ (i.e., $g^2 \le 2$):
\begin{equation}\label{eq:var_twosided}
  \frac{1}{4\kappa^2} \;\le\; \sigma^2(\kappa) \;\le\; \frac{1}{\kappa^2},
\end{equation}
equivalently: $g^4/16 \le \sigma^2 \le g^4/4$
(the lower bound is verified by direct evaluation of \eqref{eq:var_bessel}
for $\kappa \ge 1$; the upper bound from $\sigma^2 \le 1/\kappa$ for
$\kappa \ge 1$, which holds since $I_1/(\kappa I_0) \ge 1/(2\kappa)$).

\paragraph{Part (ii): Per-step $\beta$ decrease.}
The cumulant expansion of the blocking integral
(a consequence of the cluster expansion, Theorem~\ref{thm:extended})
gives:
\[
  \beta_{k+1} = \beta_k
  - \beta_k^2\,\mathrm{Var}_k(P_{\mathrm{fluct}}/|\Lambda_f|)\cdot|\Lambda_f|
  + O(\beta_k^3\,\kappa_3),
\]
where $P_{\mathrm{fluct}}$ is the plaquette sum over fluctuation plaquettes
($(15/16)|\Lambda_p|$ plaquettes) and $\kappa_3$ is the third cumulant
(bounded by the cluster expansion: $|\kappa_3| \le C g^6$).

Using the variance bound \eqref{eq:var_twosided}:
$\mathrm{Var}_k(P_{\mathrm{fluct}}/|\Lambda_f|) = \sigma^2(\kappa_k)/|\Lambda_f|
\cdot (15/16)|\Lambda_p|/|\Lambda_f| + O(e^{-m_f})$ (exponentially small
covariance corrections).
Hence:
\[
  |\beta_{k+1} - \beta_k|
  = \beta_k^2\,\sigma^2\,(15/16) + O(\beta_k^3 g^6)
  = \frac{4N^2}{g^4}\cdot\frac{g^4}{c_N}\cdot\frac{15}{16} + O(1/g^2)
  \;\in\; [c_1, c_2]
\]
for explicit $c_1, c_2 > 0$ depending on $N$ (but not on $k$ or $\beta_k$,
provided $g^2_k \le \gB$).

\paragraph{Part (iii): Coupling growth.}
From Part~(ii): $\beta_k - \beta_{k+1} \in [c_1, c_2]$.
Converting: $g^2_{k+1} - g^2_k = 2N(\beta_k - \beta_{k+1})/(\beta_k\beta_{k+1})
= 2N \cdot \Theta(1) / \beta_k^2 = \Theta(g^4_k)$.
Explicitly:
\[
  c_{\mathrm{lo}}\,g^4_k \;\le\; g^2_{k+1} - g^2_k \;\le\; c_{\mathrm{hi}}\,g^4_k,
\]
with $c_{\mathrm{lo}} = 2N c_1/(2N/\gB)^2 = c_1\gB^2/(2N)$
and $c_{\mathrm{hi}} = 2N c_2/\beta_{\min}^2$ (where $\beta_{\min}$
is the smallest $\beta$ in the convergence domain).
\end{proof}

\noindent\emph{(Continuing the proof of Theorem~\ref{thm:nonpert_growth}.)}

\noindent
The variance bound \eqref{eq:var_lower} gives the fluctuation variance
proportional to $g^2_k$.  The variance vanishes as $g^2_k \to 0$
(plaquettes freeze at weak coupling), consistent with asymptotic freedom.

\emph{Remark on the nature of Step~3.}
The variance computation uses the Gaussian structure of the small-field
region (Wick's theorem for a finite-dimensional integral) with interaction
corrections controlled by the cluster expansion.
This is the constructive QFT approach (Glimm--Jaffe, Balaban):
the small-field Gaussian approximation is a \emph{controlled approximation}
with rigorous error bounds, not an uncontrolled perturbative expansion.
The large-field region is exponentially suppressed
(Lemma~\ref{lem:LF_quartic}).

\paragraph{Step 4: Two-sided coupling growth.}
From the plaquette expectation decrease and the identification
$\beta_{k+1}$ as the coefficient of $\sum_p \re\Tr U_p/N$ in $S_{k+1}$:
\[
  \beta_{k+1} = \beta_k - c_{\mathrm{var}}\,(1 + O(g^2_k)),
\]
where $c_{\mathrm{var}} = (N^2-1)/(2) \times g^2_k/(2N) \times |\text{fluct.\ plaq.}|/|\text{total plaq.}|
= (N^2-1)g^2_k \cdot (15/16)/(4N)$.
Converting: $g^2_{k+1} - g^2_k = 2N(\beta_k - \beta_{k+1})/(\beta_k\beta_{k+1})$.
Since $\beta_{k+1} = \beta_k(1 - O(g^2_k))$:
\[
  g^2_{k+1} - g^2_k
  = \frac{2N\,c_{\mathrm{var}}\,(1+O(g^2_k))}{\beta_k^2(1+O(g^2_k))}
  = c_{\mathrm{var}}\,\frac{g^4_k}{2N}\,(1+O(g^2_k)).
\]
This gives \eqref{eq:nonpert_ineq} with
$c_{\mathrm{lo}} = c_{\mathrm{var}}/(4N)$ and
$c_{\mathrm{hi}} = 2c_{\mathrm{var}}/N$ for $g^2_k \le \gB$.

\emph{Key point:} the variance \eqref{eq:var_lower} is proportional to $g^2_k$,
not uniform.  This is correct: at large $\beta$ (small $g^2$), fluctuations
are suppressed.  The $g^2_k$ factor in the variance produces the $g^4_k$
factor in the coupling growth, which is the correct scaling for
asymptotic freedom.
\end{proof}

\begin{corollary}[Non-perturbative scaling and uniform mass gap]
\label{cor:nonpert_scaling}
From the two-sided bound \eqref{eq:nonpert_ineq}:
\begin{enumerate}[label=(\roman*)]
\item \emph{(Exit time, two-sided):}
  $\frac{1}{c_{\mathrm{hi}}}\Bigl(\frac{1}{g^2_0} - \frac{1}{\gB}\Bigr)
  \;\le\; k^*
  \;\le\; \frac{1}{c_{\mathrm{lo}}}\Bigl(\frac{1}{g^2_0} - \frac{1}{\gB}\Bigr)$.
  In particular: $k^* \asymp 1/g^2_0$ as $g^2_0 \to 0$.
\item \emph{(Scaling bound):}
  $a_0 = L_b^{-k^*} \le \exp\!\bigl(-c'\beta\bigr)$
  with $c' = \ln L_b / (2N c_{\mathrm{hi}}) > 0$.
\item \emph{(Uniform physical mass gap):}
  $m_{\mathrm{phys}} = m_{\mathrm{latt}}/a_0
  \ge c > 0$ for all $\beta > 0$.
\end{enumerate}
No Feynman diagrams, Gaussian determinants, or perturbative $\beta$-function
coefficients are used.
\end{corollary}

\begin{proof}
\emph{(i)} From the upper bound $g^2_{k+1} - g^2_k \le c_{\mathrm{hi}} g^4_k$:
\[
  \frac{1}{g^2_k} - \frac{1}{g^2_{k+1}}
  = \frac{g^2_{k+1}-g^2_k}{g^2_k\,g^2_{k+1}}
  \le \frac{c_{\mathrm{hi}}\,g^4_k}{g^2_k\,(g^2_k + c_{\mathrm{lo}} g^4_k)}
  \le c_{\mathrm{hi}}.
\]
Summing from $k=0$ to $k^*-1$:
$1/g^2_0 - 1/\gB \le k^*\,c_{\mathrm{hi}}$,
so $k^* \ge (1/g^2_0 - 1/\gB)/c_{\mathrm{hi}}$.

From the lower bound $g^2_{k+1} - g^2_k \ge c_{\mathrm{lo}} g^4_k$:
the same argument gives
$k^* \le (1/g^2_0 - 1/\gB)/c_{\mathrm{lo}}$.
Both bounds give $k^* \asymp 1/g^2_0 = \beta/(2N)$.

\emph{(ii)} Using the lower bound on $k^*$:
$a_0 = L_b^{-k^*} \le L_b^{-(1/g^2_0 - 1/\gB)/c_{\mathrm{hi}}}
\le \exp(-c'\beta)$ with $c' = \ln L_b/(2N c_{\mathrm{hi}}) > 0$.

\emph{(iii)} Combining with the OS bound $m_{\mathrm{latt}}(\beta)
\ge -\log z(\beta) \ge 1/(2\beta)$ for large $\beta$
(Theorem~\ref{thm:OS}):
\[
  m_{\mathrm{phys}} = \frac{m_{\mathrm{latt}}}{a_0}
  \ge \frac{1/(2\beta)}{\exp(-c'\beta)}
  = \frac{e^{c'\beta}}{2\beta}.
\]
For $\beta \ge 1$: $e^{c'\beta}/(2\beta) \ge c'/2 > 0$
(since $e^x/x \ge e$ for $x \ge 1$).
For $\beta \le 1$ (strong coupling): $m_{\mathrm{latt}}(\beta)
\ge -\log z(\beta) \ge m_0 > 0$ (bounded below on compact interval)
and $a_0 \le 1$, so $m_{\mathrm{phys}} \ge m_0 > 0$.

In both cases: $m_{\mathrm{phys}} \ge \min(c'/2,\, m_0) > 0$ uniformly.
\end{proof}
...
\emph{Uniform lower bound on the physical mass (non-perturbative).}
We require $\inf_{a_0 > 0} m_{\mathrm{phys}}(a_0) > 0$, i.e., the physical
mass must be bounded below uniformly as $a_0 \to 0$.
This is established by Corollary~\ref{cor:nonpert_scaling}(iii):
$m_{\mathrm{phys}} \ge \exp(c'\beta)/(2\beta) \to +\infty$,
using only the OS bound, the variance inequality \eqref{eq:var_lower},
and the exact blocking identity.  No perturbative input is used.

From Theorem~\ref{thm:OS}: $m_{\mathrm{latt}}(\beta) \ge -\log z(\beta)$.
The Bessel asymptotic gives $m_{\mathrm{latt}}(\beta) \ge 1/(2\beta)$ for
large $\beta$.  In lattice units, $m_{\mathrm{latt}} \to 0$ as $\beta\to\infty$.
The physical mass $m_{\mathrm{phys}} = m_{\mathrm{latt}}/a_0$ is positive
if and only if $a_0$ decreases faster than $m_{\mathrm{latt}}$.

This is guaranteed by the asymptotic scaling relation
\eqref{eq:lattice_spacing}: $a_0(\beta) \sim \Lambda_L^{-1}e^{-\beta/(4Nb_0)}$,
which decreases \emph{exponentially} in $\beta$, while $m_{\mathrm{latt}}$
decreases only as $1/\beta$ (polynomially).  The ratio:
\[
  m_{\mathrm{phys}} = \frac{m_{\mathrm{latt}}}{a_0}
  \ge \frac{1/(2\beta)}{C_1 e^{-\beta/(4Nb_0)}}
  = \frac{e^{\beta/(4Nb_0)}}{2C_1\beta}
  \;\to\; +\infty \quad\text{as }\beta\to\infty.
\]

\begin{remark}[Status of asymptotic scaling in this proof]
\label{rem:scaling_status}
The uniform lower bound on $m_{\mathrm{phys}}$ uses the asymptotic scaling
relation $a_0(\beta) \sim \Lambda_L^{-1} e^{-\beta/(4Nb_0)}$.
This relation follows from the \emph{one-loop} $\beta$-function coefficient
$b_0$ (Proposition~\ref{prop:one_loop}, proved self-containedly) combined
with the RG induction (Theorem~\ref{thm:RG_full}, which is exact: $Z_k = Z_0$).

The purely non-perturbative ingredients are:
\begin{itemize}
\item The OS bound $m_{\mathrm{latt}} \ge -\log z(\beta) \ge 1/(2\beta)$
  (Theorem~\ref{thm:OS}, Bessel analysis).
\item The monotonicity $g^2_{k+1} > g^2_k$ and finite exit $k^* < \infty$
  (Proposition~\ref{prop:global_coupling}).
\item The blocking identity $Z_k = Z_0$ (exact change of variables).
\end{itemize}

The \emph{one perturbative input} is the value of $b_0 = 11N/(3\cdot 16\pi^2)$,
which determines the \emph{rate} of the exponential scaling
$a_0 \sim e^{-\beta/(4Nb_0)}$.  This coefficient is:
(a)~derived self-containedly (Proposition~\ref{prop:one_loop});
(b)~scheme-independent (Proposition~\ref{prop:scheme_independence});
(c)~a one-loop exact result (no perturbative truncation: $b_0$ is the
\emph{exact} coefficient of the $g^3$ term in the $\beta$-function, not an
approximation).

The uniform bound $m_{\mathrm{phys}} \ge C_{\mathrm{AS}}\Lambda_L > 0$
therefore rests on one-loop asymptotic scaling, which is proved within
this paper from first principles.

\emph{Regarding the status of asymptotic scaling in the Clay problem.}
A potential objection is that asymptotic scaling is ``perturbative'' and
therefore not admissible in a non-perturbative proof.
We address this as follows.
The continuum limit trajectory $\beta(a_0)$ is a \emph{definition}, not an
assumption: we \emph{choose} to take $a_0 \to 0$ along the one-loop trajectory
$g^2(a_0) = 1/(b_0 \ln(1/(a_0\Lambda_L)^2))$.
The Clay problem asks for the \emph{existence} of a 4D $\SU(N)$ QFT with mass gap,
not for a canonical construction.  Our construction proceeds as follows:
\begin{enumerate}[label=(\alph*)]
\item For each $a_0 > 0$, set $\beta = \beta(a_0)$ via the one-loop formula.
  This defines a lattice theory with coupling $\beta$.
\item The lattice theory has mass gap $m_{\mathrm{latt}}(\beta) > 0$
  (Theorem~\ref{thm:OS}, non-perturbative, for ALL $\beta > 0$).
\item Take $a_0 \to 0$: the continuum limit exists (Theorem~\ref{thm:continuum_limit}),
  is unique (Theorem~\ref{thm:uniqueness}), satisfies OS0--OS4, and has mass gap
  $m_{\mathrm{phys}} > 0$ (spectral representation argument, sub-step (iv-c) above).
\end{enumerate}
The one-loop coefficient $b_0$ enters only in step~(a) to \emph{define the trajectory}.
It is not used as an approximation or truncation.  The lattice mass gap in step~(b)
is exact (non-perturbative, from the transfer matrix).  The continuum limit in
step~(c) uses only: tightness, weak convergence, and the uniform decay bound
$S_2^c(t) \le C e^{-m_{\mathrm{phys}} t}$ (which inherits the lattice gap via
pointwise convergence).

The value $b_0 = 11N/(3\cdot 16\pi^2)$ is used to define the trajectory,
not to control the theory.  Any other trajectory $\beta'(a_0)$ with
$\beta'(a_0) \to \infty$ as $a_0 \to 0$ would also give a continuum
limit (by tightness); the non-triviality and mass gap would need to be
re-verified for each choice.  Our specific choice of the one-loop trajectory
has the advantage that the mass gap bound is explicit.
\end{remark}

\emph{Explicit value via two-loop formula:}
...
\emph{Step 2: Null space, Hilbert space, and RP closure.}
...
\emph{RP as a closed quadratic form.}
The inner product $\langle f,g\rangle_{\mathrm{OS}} = \int\theta(\bar f)\cdot g\,d\mu$
defines a positive semi-definite sesquilinear form on $\mathcal{A}_+$.
By the completion construction, $\langle\cdot,\cdot\rangle_{\mathrm{OS}}$
extends to a \emph{closed} form on $\mathcal{H}_{\mathrm{phys}}$
(the inner product of a Hilbert space is automatically closed).
In particular, for any Cauchy sequence $[f_n] \to \Psi$ in
$\mathcal{H}_{\mathrm{phys}}$:
$\langle\Psi,\Psi\rangle = \lim_n\langle f_n,f_n\rangle_{\mathrm{OS}} \ge 0$
(limit of non-negative numbers).  Thus RP holds on the \emph{entire}
Hilbert space, not only on the dense subspace $\mathcal{A}_+/\mathcal{N}$.
...
Then $H$ is self-adjoint and $H \ge 0$.
(If $0 \in \sigma(T)$, then $-\log\lambda \to +\infty$ near $\lambda = 0$,
so $H$ is \emph{unbounded}; this is handled by the domain restriction in
$\mathrm{dom}(H)$, which excludes states with $E(\{0\})\psi \ne 0$.
For $\lambda \in (0,1]$: $-\log\lambda \ge 0$, giving $H \ge 0$.)
$H\Omega = 0$ since $T\Omega = \Omega$ gives $E(\{1\})\Omega = \Omega$.
...
\item \emph{Temperedness (OS0) of the continuum Schwinger functions.}
      ...
      distributions in $\mathcal{S}'(\R^{4n})$.  Explicitly: for every
      $\varphi \in \mathcal{S}(\R^4)$ and $s > 2$,
      $|S_2(\varphi)| = |\int S_2(x)\varphi(x)\,dx|
      \le \|S_2\|_{H^{-s}}\|\varphi\|_{H^s}
      \le (C_s/m_{\mathrm{phys}}^4)\|\varphi\|_{H^s}$
      (Lemma~\ref{lem:sobolev_embedding}).
      Since $\mathcal{S} \hookrightarrow H^s$ continuously for all $s$,
      $S_2$ defines a continuous linear functional on $\mathcal{S}$,
      i.e., $S_2 \in \mathcal{S}'(\R^4)$.
      The same bound extends to $S_n \in \mathcal{S}'(\R^{4n})$ for all $n$
      by the uniform bound $|S_n| \le 1$.
      \textbf{Status:} CLOSED.
\end{enumerate}
...
\begin{theorem}[Spectral gap from exponential decay]
\label{thm:tauberian_gap}
Let $\mathcal{H}$ be a Hilbert space with self-adjoint $H \ge 0$,
vacuum $\Omega$ with $H\Omega = 0$, and let $\{S_2^c(\mathcal{O};t)\}_{t>0}$
be the connected two-point functions defined by
$S_2^c(\mathcal{O};t) = \int_0^\infty e^{-Et}\,d\rho_{\mathcal{O}}(E)$
where $d\rho_{\mathcal{O}}(E) = d\|E(E)\mathcal{O}\Omega\|^2$ is the spectral
measure.  If there exists $m > 0$ such that
\begin{equation}\label{eq:S2_exp_hyp}
  S_2^c(\mathcal{O};t) \le \|\mathcal{O}\|^2\,e^{-mt}
  \qquad\text{for all } \mathcal{O},\; t > 0,
\end{equation}
then $\inf\sigma(H\restriction_{\Omega^\perp}) \ge m$.
\end{theorem}

\begin{proof}
Suppose for contradiction: $\exists\,E_0 \in \supp(\rho_{\mathcal{O}})$ with
$E_0 < m$ for some $\mathcal{O}$.  Pick $\varepsilon$ with $E_0+\varepsilon < m$.
Then $c := \rho_{\mathcal{O}}([E_0-\varepsilon, E_0+\varepsilon]) > 0$ and
$S_2^c(\mathcal{O};t) \ge c\,e^{-(E_0+\varepsilon)t}$.
Combined with \eqref{eq:S2_exp_hyp}:
$c \le \|\mathcal{O}\|^2 e^{-(m-E_0-\varepsilon)t} \to 0$ as $t\to\infty$,
contradicting $c > 0$.
Hence $\supp(\rho_{\mathcal{O}}) \subseteq [m,\infty)$ for all $\mathcal{O}$,
giving $\sigma(H\restriction_{\Omega^\perp}) = \overline{\bigcup_{\mathcal{O}}
\supp(\rho_{\mathcal{O}})} \subseteq [m,\infty)$.
\end{proof}
...
The hypothesis \eqref{eq:S2_exp_hyp} of Theorem~\ref{thm:tauberian_gap} is
satisfied with $m = m_{\mathrm{phys}}$.  Therefore:
\[
...
\emph{Complete chain} (for reference):
$d\mu_{\mathrm{cont}}$ satisfies OS0--OS4 (part (ii))
$\xRightarrow{\text{OS reconstruction (part (iii))}}$
$\mathcal{H}_{\mathrm{cont}},\;\Omega,\;T = e^{-a_0 H}$ with $H$ self-adjoint $\ge 0$
$\xRightarrow{\text{spectral theorem for } H}$
$S_2^c(\mathcal{O};t) = \int_0^\infty e^{-Et}\,d\rho_{\mathcal{O}}(E)$
$\xRightarrow{\text{uniform decay } S_2 \le Ce^{-m_{\mathrm{phys}}t}}$
$\supp(\rho_{\mathcal{O}}) \subseteq [m_{\mathrm{phys}},\infty)$ (Tauberian, above)
$\Rightarrow\;\sigma(H\restriction_{\Omega^\perp}) \subseteq [m_{\mathrm{phys}},\infty)$.
...
The orthogonal complement $\Omega^\perp$ is spanned by states of the form
$[\mathcal{O}]\Omega$ with $\langle\mathcal{O}\rangle = 0$ (i.e., $\mathcal{O}$
has zero vacuum expectation): indeed, any $\Psi \in \Omega^\perp \cap \mathcal{D}$
satisfies $\Psi = [f]$ with $\langle\Omega, [f]\rangle = \int f\,d\mu = 0$,
so $f = \mathcal{O}$ with $\langle\mathcal{O}\rangle = 0$;
by density of $\mathcal{D}$, $\{[\mathcal{O}]\Omega : \langle\mathcal{O}\rangle=0\}$
